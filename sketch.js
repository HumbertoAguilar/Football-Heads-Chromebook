...
//int=createSprite(1360,50)
  ...
  homen.position(1275,50);
buton=createButton('| |')
buton.position(1220,560)
  buton2=createButton('? ')
  buton2.position(1293,560);
...
if(keyDown("a")&&car2.position.y>265){
...
if(keyDown("d")&&car2.position.y>265){
  //Body.applyForce(car2.bodies[0],{x:car2.bodies[0].position.x, y:car2.bodies[0].position.y},{x:+0.03,y:0})
  Body.setVelocity( car2, {x:5, y: 0});
  Matter.Body.setAngle(car2, 0)
...
if(keyDown("w")&&car2.position.y>305){
...
if(keyWentDown('r')&&ball.position.x<car2.position.x+100&&ball.position.y>330){
  Body.applyForce(
    ball,
...
if(keyWentDown('m')&&ball.position.x>car.position.x-100&&ball.position.y>330){
  Body.applyForce(
    ball,
    { x: ball.position.x, y: ball.position.y },
    { x: -0.06, y: -0.065}
...
if(ball.position.x>1050&&ball.position.y>200&&ball.position.y<500){
score+=+1
goalSound.play()
...
function controls(){
  clickSound.play()
  location.replace('  https://humbertoaguilar.github.io/ControlsP2 ');
